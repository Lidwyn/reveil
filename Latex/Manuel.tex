...
Le réveil permet l'affichage et le réglage d'heure. Il permet de configurer plusieurs réveils en simultané, et les enregistres même quand il ne sont pas actifs.
Par exemple, un réveil à 8h les lundis, mardis et mercredis, et un autre à 8h15 les jeudis et vendredis. Les révéils peuvents aussi être à usage unique et s'auto désarmer ensuite.
...
\begin{itemize}
    ...
    \item non affiché : un large bouton sur le dessus permettant l'arrêt de l'alarme ou le control de l'inibition de l'affichage
\end{itemize}

\section{Réglage de l'heure}

Pour le réglage de l'heure, il faut tout d'abord appuyer sur le bouton setup (bouton S).
Lorsqu'on appui sur le bouton setup, l'affichage de l'heure se met à clignoter et les secondes sont affichées sur l'écran de droite.
Lorsqu'un élément cligonte (l'heure par défaut), on peut le modifier avec les flèches directionnelle haute et basse.
Pour passer d'un éléments à un autre, on utilise les flèches de gauche et de droite. Nous avons dans l'ordre:

Heures $\leftrightarrow$ Minutes $\leftrightarrow$ Secondes $\leftrightarrow$ Jours $\leftrightarrow$ Mois $\leftrightarrow$ Année
...
le mois sur les deux derniers digits de l'affichage principal et l'année sur l'affichage secondaire.

Pour le réglage des secondes, on appui sur le bouton "haut", à ce moment le clignotement s'arrête
et une seconde est sélectionnée : le prochain 0, 15, 30 ou 45 et on peut ajouter 15 sec en appuyant à nouveaux sur ce bouton.
On peut aussi en retier avec le bouton "bas", le bouton "gauche permet d'annuler la modification" et le bouton gauche permet de force la seconde de l'appareil à celle affichée.
Cela permet de synchroniser l'appareil avec un horloge externe. En pratique on met la seconde sur une seconde qui n'est pas encore arrivé,
on attend avec une autre horloge et au moment ou l'on arrive sur cette seconde avec la deuxième horloge, on clique sur le bouton gauche et les deux seront synchronisées.

\begin{enumerate}
    \item appuyer sur le bouton menu, l'affichage de l'heure clignote, et l'affichage du prochain reveil passe à l'affichage des secondes.
    \item reglage de l'heure (et des autres paramètres à part les secondes) avec les boutons haut et bas
    \item selections du paramètre via les touches gauche et droite dans l'ordre : Heures $\leftrightarrow$ Minutes $\leftrightarrow$ Secondes $\leftrightarrow$ Jours $\leftrightarrow$ Mois $\leftrightarrow$ Année
    \item lorsque l'on clique sur le bouton droite alors que ce sont les secondes qui sont séléctionnés, on passe sur l'affichage des réglage bis, comme ceci:
\end{enumerate}

\subsection{appuyer sur le bouton menu}

Pour commencer on appui sur le bouton menu/setup \circled{\textbf{A}}. 
On passe en mode réglage, et l'affichage de l'heure clignote \circled{\textbf{B}}, et l'affichage du prochain reveil passe à l'affichage des secondes\circled{\textbf{C}}.
...
\begin{figure}[h!]
    ...
    \begin{annotatedFigure}
        ...
        \annotatedFigureBox{.661}{.115}{.311}{.4}{C}{8}[red]
...
\subsection{reglage des paramètres}

Dans ce mode on peut régler les paramètres lorsqu'ils sont sélectionnés. Pour se faire, il faut utiliser les bouton haut et bas \circled{\textbf{D}}.
Sauf pour les secondes ou le réglage est différent et sera expliqué plus tard.
...
Il y a 6 paramètres réglables dans l'ordre :
\begin{itemize}
...
lorsqu'on est sur les secondes et qu'on appui sur le bouton droite, on passe sur l'affichage 2 avec les paramètres restant.
Pour revenir au premier écran, il faut appuyer sur le bouton gauche lorqu'on est sur les jours.
...
\subsection{Réglage des secondes}
